# Workspace Subdomain Auth — Implementation Plan

> **Superseded:** Conteúdo TDD mesclado no plano canônico  
> [`cursor_workspace-subdomain-auth.md`](./cursor_workspace-subdomain-auth.md)  
> (Parte I = este arquivo; Parte II = addendum Tasks 12–15).  
> Mantido como referência histórica / diff de commits.

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Each workspace gets its own subdomain (`<slug>.agentserver.analytics.vtex.com`). Login from that subdomain is locked to that workspace — the session is created already bound to it (no separate "select workspace" step).

**Architecture:** Backend gains a `slug` column on `workspaces` (unique, kebab-case, validated against reserved words). The login handler accepts an optional `workspace_slug`; when present, it resolves to a workspace, validates the user is a member, and stamps `active_workspace_id` on the new session token. Frontend parses the hostname's first label and injects it on the login request. Workspace creation generates a slug from the name with collision suffixing. Sandbox subdomains (`claw-*`, `hermes-*`) keep their existing routing — workspace slugs are reserved against those prefixes.

**Tech Stack:**
- Backend: Go 1.26, `github.com/agentserver/agentserver/internal/{auth,db,server}`
- DB: PostgreSQL (sql-migrate style migrations under `internal/db/migrations/`)
- Frontend: React 19 + TypeScript + Vite (`web/src/`)
- Tests: Go `testing` + integration with `TEST_DATABASE_URL`; React Testing Library for components
- Build tag: `goolm` required (already in CI)

**Predecessor:** PR #53 added `active_workspace_id` on `auth_tokens`. This plan reuses `SetTokenActiveWorkspace` to stamp the workspace at login time instead of a separate call.

**Scope (Opção A only):** No SSO, no SAML, no per-workspace IdP. Those are Opção B/C and remain in `docs/workspace-auth-design.md` for future work.
