...
**agentserver 侧（新增）：**
- `POST /api/workspaces/{wid}/tui/inbound` — TUI 上传 prompt。
- `POST /api/agent-sessions` — 显式新建 session（`/clear`）。
- `POST /api/agent-sessions/{sid}/attach` — 重新认领 permission_responder。
- `GET  /api/agent-sessions/{sid}/events` — SSE 长连接（如已有，复用）。
- `POST /api/agent-sessions/{sid}/control` — R 类 slash 命令路由。
- `POST /api/agent-sessions/{sid}/turns/{tid}/cancel` — 中断 turn。
- `POST /api/agent-sessions/{sid}/permissions/{pid}` — 权限决定。
- `GET  /api/agent-sessions?owner=...&latest=1` — `--continue` 用。
- IMRouter 注册 `tui` provider：no-op send。

**cc-broker 侧（新增）：**
- `internal/ccbroker/tools/permission.go` — 闸逻辑。
- `internal/ccbroker/tools/prompt.go` — system prompt builder。
- `handler_turns.go`：metadata 解析 + 透传。
- `runner/options.go`：metadata → SDK options 映射。
- `tools/executor.go`：每个 `remote_*` handler 入口套 `gate.Check`。
- 新转发端点：`/api/sessions/{sid}/turns/{tid}/cancel`、`/api/sessions/{sid}/permissions/{pid}/decide`、`/api/sessions/{sid}/compact`。
...
User    TUI(BubbleTea)  TUI(HTTPCli)   TUI(SSE)   agentserver    cc-broker    exec-registry   TUI(Executor)
 ...
 │           │              │  POST /tui/inbound                      │              │              │
 │           │              ├────────────────────────►│              │              │              │
 │           │              │             │            │  insert agent_session_events (user msg)   │
 │           │              │◄─202 Accepted───────────┤              │              │              │
 │           │              │             │            │  POST /api/turns (SSE)                    │
 │           │              │             │            ├─────────────►│              │              │
 │           │              │             │            │              │ TurnLock + workspace.Setup │
 │           │              │             │            │              │ + agentsdk.NewClient        │
...
func makeRuleKey(tool, executorID string, args json.RawMessage) string {
    ...
    case "remote_bash":
        ...
        return fmt.Sprintf("%s|%s|cmd:%s", tool, executorID, first)
...
`(tool_name, executor_id, command_first_word_or_dirname)` 三元组完全匹配则命中。批准 `git status` 后 `git push` 也命中（首词都是 `git`），用户应慎用 `always`。v1.x 加规则编辑面板让用户调粒度。
...
[用户重启 tui --continue]
   ...
   │  → tunnel 上线
   ├─ TUI 调 GET /api/agent-sessions?owner=tui:<eid>&latest=1
...
所有端点的 auth：`Authorization: Bearer <oauth_access_token>`，token 由 `EnsureValidToken(serverURL)` 维护（与 `executor` 子命令同款）。所有 5xx/4xx 响应 body 为 `{"error":{"code":"...","message":"..."}}`。

### 4.1 端点清单

| 端点 | 方法 | 调用方 | 用途 |
|---|---|---|---|
| `/api/workspaces/{wid}/tui/inbound` | POST | TUI | 提交 prompt（自动 attach 或新建 session） |
| `/api/agent-sessions` | POST | TUI | 显式新建 session（`/clear`） |
| `/api/agent-sessions/{sid}/attach` | POST | TUI | 重新认领 permission_responder |
| `/api/agent-sessions/{sid}/events` | GET (SSE) | TUI | 订阅 session 事件流 |
| `/api/agent-sessions/{sid}/control` | POST | TUI | R 类 slash 命令 |
| `/api/agent-sessions/{sid}/turns/{tid}/cancel` | POST | TUI | 中断当前 turn |
| `/api/agent-sessions/{sid}/permissions/{pid}` | POST | TUI | 权限决定 |
| `/api/agent-sessions?owner=...&latest=1` | GET | TUI | 查找最近 session（`--continue`） |
...
**幂等与并发：**
- 同 session 已有未结束 turn → 409 `{"code":"turn_in_progress","turn_id":"..."}`。
...
| `agents` | `{}` | 调 executor-registry `GET /api/workspaces/{wid}/executors` | `{"executors":[{...}]}` |
...
### 4.9 `GET /api/agent-sessions?owner=...&latest=1`

```
GET /api/agent-sessions?workspace_id=ws_xxx&owner=tui:exe_tui_a&latest=1
```
...
`owner=tui:<executor_id>` filter：external_id 以此前缀开头且 channel_type=tui。`latest=1` 只取最近活跃的一条。

### 4.10 IMRouter 注册 `tui` provider

让 `send_message` / `send_image` / `send_file` 工具能在 TUI session 上运行不报错。

```go
// internal/server/im/providers.go (示意)
type tuiProvider struct{}

func (p *tuiProvider) Send(ctx context.Context, channel Channel, jid string,
    msg OutboundMessage) error {
    // No-op: send_* 工具产生的事件已经通过 cc-broker → agent_session_events
    // → SSE 流到达 TUI。这里不需要二次外发。
    return nil
}

func (p *tuiProvider) Capabilities() Capabilities {
    return Capabilities{Text: true, Image: true, File: true}
}
```

注册：`router.Register("tui", &tuiProvider{})`。Channel 解析：TUI session 创建时自动绑定一个 `channel_type=tui` 的 channel record，credentials 为空。
...
UPDATE agent_executors SET owner_user_id = 'unknown' WHERE owner_user_id IS NULL;

CREATE INDEX IF NOT EXISTS idx_agent_sessions_external_owner
  ON agent_sessions (workspace_id, external_id);
```
...
`agent_sessions.responder_attached_at` 由 SSE 心跳维护：
- TUI 的 SSE consumer 每 30s 收到 server keepalive；server 端在每次 keepalive 时把 `responder_attached_at = now()`。
- 连续 90s 没刷新 → server 异步 worker 把 `permission_responder = NULL`。
- 同时把该 session 上所有 pending permission_request emit `permission_orphaned` 事件——cc-broker 收到自决：超时 deny。
...
**v1 选 A 方案：** TUI 启动时把 `cwd` 字段写入 `~/.agentserver/executors/<eid>.json` 的扩展字段；Executor goroutine 每次 `Execute()` 之前 stat 该文件 mtime，变了则 reload。**注意：这里破坏了一点解耦原则**——但 cwd 改动是用户从这台机器对这台机器发起的操作，没必要绕一圈到云端。
...
func optsFromMetadata(meta TurnMetadata, base []agentsdk.QueryOption) []agentsdk.QueryOption {
    ...
    if meta.CompactNow {
        // SDK 没有强 compact 开关，v1 简化：把指令注入 user_message prefix
    }
    return opts
}
```
...
| agentserver IMRouter `tui` provider | no-op send 单元测试 | `internal/server/im/tui_provider_test.go` |
...
1. **DB 迁移** —— 加字段，旧数据回填。
2. **agentserver 部署** —— 含 TUI 端点 + IMRouter `tui` provider；旧 IM 流不受影响。
3. **cc-broker 部署** —— 含 metadata 透传 + permission gate；metadata 全 nil 时走 default 行为。
4. **agent 二进制发布** —— 加 `tui` 子命令。

每步可独立 rollback：1 是 ADD COLUMN；2/3 重启后旧请求格式仍可解析；4 用户不升级则没事。
...
| **`always` 规则匹配粒度偏粗** | 中 | bash 命令首词 + 路径 dirname。粒度过粗可能误放行（如批准 `git status` 后 `git push` 也命中）。v1.x 加规则编辑面板 |
...
```
internal/agent/executor_client.go                  # 注册时加 interactive=true
internal/ccbroker/handler_turns.go                 # 解析 metadata
internal/ccbroker/runner/options.go                # metadata → SDK options
internal/ccbroker/tools/context.go                 # Context 加新字段
internal/ccbroker/tools/executor.go                # 每个 handler 入口套 Gate.Check
internal/ccbroker/server.go                        # 注册新转发端点
internal/server/...                                # TUI 端点 + IMRouter tui provider
```

### A.3 数据库迁移

```
internal/server/migrations/20260503_tui_session_fields.sql
```
