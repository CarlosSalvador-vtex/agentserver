...
type sendImageInput struct {
	// Source must be base64-encoded image bytes. The HTTP-MCP era
	...
	// is, callers must pass base64 directly. If Source is not valid base64,
	...
	return []agentsdk.McpTool{
		...
		agentsdk.Tool[sendImageInput]("send_image",
			...
			func(ctx context.Context, in sendImageInput) (*agentsdk.McpToolResult, error) {
				...
				return imbridgePost(ctx, tctx, "/api/internal/imbridge/send-image", body)
			}),
...
func imbridgePost(ctx context.Context, tctx *Context, path string, body map[string]string) (*agentsdk.McpToolResult, error) {
	if tctx.IMBridgeURL == "" {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, tctx.IMBridgeURL+path, bytes.NewReader(raw))
	if err != nil {
