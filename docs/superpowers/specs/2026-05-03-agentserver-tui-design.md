...
| 类别 | 含义 | 例子 | 实现位置 |
|---|---|---|---|
| **L（Local I/O）** | 只关乎本地进程 / 本地手脚 / 本地输入策略 / 本地认证状态 | `/quit`、`/cd <path>`、`/yolo`、`/attach <path>`、`/login`、`/logout` | TUI 进程内 |
...
**启动行为：**
- **有 saved credentials**（`~/.agentserver/credentials.json`）：自动 `EnsureValidToken`（必要时 refresh）→ `LoadOrRegisterExecutor` → 进 TUI 主界面，**已登录态**。
- **无 saved credentials**（首次跑 / 凭据被删）：**不阻塞 TUI 启动**，直接进 TUI 主界面，**未登录态**——状态栏标 `auth: logged out`，底部输入框禁用，浮动一行提示 `Use /login to authenticate`。`--server` flag 必填（否则不知道 OAuth 端点在哪）。
- **`/login` 触发流程：** §5.12 详述。

⚠️ **vs `connect` 子命令的差异：** `connect` 没有凭据时会 `log.Fatal`；`tui` 不会——保持 TUI 可用，让用户用 `/login` 在 UI 内完成授权（更接近 claude code TUI 的体验）。
...
│ auth: alice · tunnel: online · events: live · turn: idle · opus-4-7│
├────────────────────────────────────────────────────────────────────┤
│  ▸ user                                                            │
...
const (
    ...
    ModeAwaitPerm                // 权限浮层显示中
    ModeAwaitAskUser             // ask_user 面板显示中
    ModeAwaitLogin               // login (OAuth Device Flow) 浮层显示中
    ModeAwaitLogout              // logout 确认面板
...
type Model struct {
    cfg          Config
    bus          *Bus
    auth         *AuthController  // §5.12.7

    mode         Mode
    authState    AuthState        // 镜像 auth.state，用于 View 渲染
    sessionID    string
    turnID       string
    cwd          string
    model        string
    ...
    statusAuth   string          // logged out | <user_id> | logging in | refreshing
    ...
    loginPanel   *LoginPanel       // 仅 AwaitLogin 时非 nil
    logoutPanel  *LogoutPanel      // 仅 AwaitLogout 时非 nil
...
// 认证（§5.12）
type DeviceCodeReadyMsg struct{ UserCode, VerifyURL string; ExpiresIn, Interval int }
type LoginPollMsg       struct{ State string; Err error }   // pending|slow_down|denied|expired|success
...
type AuthStateChangedMsg struct{ From, To AuthState; Reason string }
```
...
| 键 | Mode | 行为 |
|---|---|---|
| `Enter` | Normal（input 焦点，AuthState=LoggedIn） | 发送 prompt |
| `Enter` | Normal（AuthState=LoggedOut） | 输入框禁用，按下无反应 |
| `Shift+Enter` | Normal | 输入框换行 |
| `Esc` | Normal | turn 在跑 → 调 cancel；否则清空当前输入 |
| `Esc` | AwaitPerm/AwaitAskUser/AwaitLogin | 暂存当前面板，返回 Normal（login 浮层 Esc 取消整个登录流程） |
| `Ctrl+C` | 任何 | 二次按下退出 |
| `Ctrl+T` | Normal | 打开 session switcher |
| `/` | Normal（input 为空，任何 AuthState） | 打开命令面板（LoggedOut 时只显示 L 类命令） |
| `?` | Normal（input 为空） | 显示键位帮助浮层 |
| `PgUp/PgDn` | Normal | viewport 翻页 |
| `Home/End` | Normal | viewport 跳顶/底 |
| `Enter` | Tool block 选中态 | 展开/折叠 |
| `o` | AwaitLogin | 调 `browser.OpenURL` 打开 verify URL |
...
- L 类直接在 Update 里处理（如 `/quit` → `tea.Quit`，`/login` → `auth.StartLogin`，`/logout` → `auth.Logout`）。
- S 类调对应的 Bus 方法（**要求 AuthState=LoggedIn**，否则报错 `not authenticated; use /login first`）。
- R 类直接打包成 `Bus.PostControl(name, args)`，**不本地解释**——agentserver 知道。同样要求 LoggedIn。

**L 类完整清单（v1）：** `/quit`、`/cd`、`/yolo`、`/attach`、`/login`、`/logout`、`/help`。

**关键设计：** L/S 类是写死在 TUI 里的，R 类全靠 server 端注册——TUI 解析 `/` 命令时，只校验"是不是已知 L/S 名称"，否则全部当作 R 类透传。新增 `/system-prompt-foo` 之类的 R 命令时 TUI 零改动。

**LoggedOut 状态下的命令面板：** 只列 L 类命令（且 `/cd`/`/yolo`/`/attach` 这些"需 executor 在线"的也灰显），最显眼位置是 `/login`。`/help` 列出 L 类完整清单 + 提示 "log in to access more commands"。

`/help` 列命令清单时硬编码 R 类 `[model, permission, compact, cost, agents]`；v1.x 加 `GET /api/control/commands` 让 server 报清单。
...
type AuthState int
const (
    AuthLoggedOut    AuthState = iota  // 无 credentials 或 refresh 失败
    AuthLoggingIn                       // OAuth Device Flow 进行中
    AuthLoggedIn                        // 有有效 token
    AuthRefreshing                      // 后台刷 token，UI 不阻塞
)
```

UI 行为按状态分支：

| AuthState | 状态栏 | 输入框 | SSE 订阅 | Executor goroutine |
|---|---|---|---|---|
| `LoggedOut` | `auth: logged out` 红色 | 禁用，底部提示 `Use /login to authenticate` | 不订阅 | 不启动 |
| `LoggingIn` | `auth: logging in...` 黄色 + 浮层 | 禁用 | 不订阅 | 不启动 |
| `LoggedIn` | `auth: <user_id>` 绿色 | 启用 | 订阅 | 启动 |
| `Refreshing` | `auth: <user_id> (refreshing)` 黄色 | 启用（请求会等 refresh 完） | 维持 | 维持 |

状态转换：

```
启动 ──┬─ 有有效 creds ───────────────────► LoggedIn
       │
       └─ 无 creds / refresh failed ─────► LoggedOut
                  │
                  │ /login
                  ▼
              LoggingIn ──┬─ success ──► LoggedIn
                          ├─ user cancel/timeout ──► LoggedOut
                          └─ deny by server ──► LoggedOut

LoggedIn ──┬─ token 即将过期 ──► Refreshing ──┬─ refresh ok ──► LoggedIn
           │                                  └─ refresh fail ──► LoggedOut (清 creds)
           │
           └─ /logout ──► LoggedOut (清 creds)

LoggedOut/LoggedIn 期间：401/403 from any TUI endpoint ──► 触发 token refresh
                                                          (transitioning Refreshing)
```

#### 5.12.2 `/login` 命令

L 类，在 TUI 进程内启动 OAuth Device Flow（复用 `internal/agent/login.go` 的 `requestDeviceCode` + `pollForToken`，**但 UI 输出走 Bubble Tea，不输出 stdout/stderr**）。

**触发：** `/login` 在 `LoggedOut` 时可用；在 `LoggedIn` 时返回 `already logged in as <user>; use /logout first`；在 `LoggingIn` 时返回 `login already in progress`。

**Server URL 来源：** `--server` flag → saved credentials.ServerURL → 弹一个输入子面板让用户填（v1 简化：`--server` 必填，无则报 `/login requires --server flag at startup` 后回 LoggedOut）。

**UI 流程：**

1. TUI 进 `LoggingIn` 状态。
2. `bus.RequestDeviceCode(serverURL)` 异步执行 → 返 `DeviceCodeReadyMsg{userCode, verifyURL, expiresIn, interval}`。
3. 弹**login 浮层**（与 permission 浮层共享 panel 机制）：

   ```
   ╭─ Authenticate to agentserver ───────────────────────────────╮
   │                                                              │
   │   Visit:    https://agent.cs.ac.cn/oauth2/device/verify     │
   │   Code:     ABCD-WXYZ                                        │
   │                                                              │
   │   ┌──────────────────────┐                                  │
   │   │  ▄▄▄▄▄▄▄  ▄ ▄ ▄▄▄▄▄  │   (QR code, half-block 渲染,    │
   │   │  █ ▄▄▄ █  ██▄ █ ▄ █  │    与 login.go 的 qrterminal     │
   │   │  █ ███ █   ▄ ███▄ █  │    保持一致风格)                │
   │   │   ...                │                                  │
   │   └──────────────────────┘                                  │
   │                                                              │
...
4. 后台 goroutine 跑 `pollForToken`，按 server 返回的 `interval` 轮询；每次结果 emit `LoginPollMsg{state}`：
   - `authorization_pending` → 浮层倒计时刷新，继续等
   - `slow_down` → 调慢 interval，继续等
   - `access_denied` → 浮层关闭，状态回 `LoggedOut`，timeline emit 一行 `login denied`
   - `expired_token` → 浮层关闭，回 `LoggedOut`，timeline `login code expired, try /login again`
   - `success` → 走步骤 5
5. **登录成功：** `SaveCredentials`（同 `RunLogin`）→ TUI 状态进 `LoggedIn` → 触发"延迟启动序列"（§5.12.3）→ 浮层关闭 + timeline emit `Logged in as <user>; registering executor...`。

**键位：**
- `o` → `browser.OpenURL(verifyURL)`（同 `RunLogin`，但不强制：用户已经看到 QR 和 URL 了）。
- `Esc` → 取消，状态回 `LoggedOut`，pollForToken goroutine 收 ctx.Done 退出。
- `Ctrl+C` × 2 → 同正常退出，期间也能退。

**与 `--skip-open-browser` 的关系：** `tui --skip-open-browser` 启动时把 `LoginPanel.AutoOpenBrowser = false`，浮层不自动开浏览器，仅显示 QR + URL。等同于 `o` 键不被默认按下。

#### 5.12.3 登录后的"延迟启动序列"

`LoggedOut → LoggedIn` 状态切换触发：

```
1. 启动 ExecutorClient 协程
   ├─ LoadOrRegisterExecutor(opts)  ← 同 executor 子命令，可能要 register 或复用 saved
   ├─ tunnel 上线
   └─ 状态栏 `tunnel: online`
2. 解析 session
   ├─ if --resume <sid>: bus.AttachSession(sid)
   ├─ elif --continue:   bus.FetchLatestSession() then attach
   └─ else: 不主动建 session，等用户首次 prompt 时由 inbound 隐式新建
3. 开始 SSE 订阅（如已有 sessionID）
4. timeline emit `Ready. Type a message to start.`
```

如果第 1 步注册失败（如 token 立刻被吊销），状态回 `LoggedOut` + timeline 显示原因。

#### 5.12.4 `/logout` 命令

L 类。当前 `LoggedIn` 时可用：

1. 弹确认面板：`Logout? Local credentials and executor session will be cleared. [y/N]`
2. 确认 → cancel current turn（如有）→ stop SSE → stop ExecutorClient → 删 `~/.agentserver/credentials.json` → 删 `~/.agentserver/executors/<eid>.json` → 状态回 `LoggedOut` → timeline `Logged out.`
3. TUI 不退出——用户可以紧接着 `/login` 重新登录。

#### 5.12.5 Token 自动刷新与 401 处理

`Bus` 的每个 HTTP 请求执行前调 `EnsureValidToken`：
- **token 还有 > 5min 有效期** → 直接用，状态保持 `LoggedIn`。
- **token 即将过期 / 已过期但有 refresh_token** → 状态进 `Refreshing` → 调 refresh endpoint → 成功转回 `LoggedIn`，请求继续；失败则清 creds，状态进 `LoggedOut`，本次请求返 `auth_required` 错误 → TUI 在 timeline emit `Token expired. Use /login to re-authenticate.` 弹一条 banner。
- **服务器回 401/403** → 同 refresh 失败路径处理。

`LoggedOut` 之后：进行中的 turn 会随 SSE 断开而停止；用户重新登录后 `/resume <sid>` 可继续。

#### 5.12.6 与多 TUI 场景的交互

- 同一台机器同账号起两个 `tui`：第二个起来时也 `LoadOrRegisterExecutor`——会得到不同的 `executor_id`（`(account, hostname-pid)` 区分），各自独立 attach 自己的 session。两个共享同一份 `credentials.json`（refresh token 文件锁防并发写，已由 `internal/agent/credentials.go` 处理；如未处理需补，见附录 B）。
- `/logout` 一处会**影响所有同进程的实例**：一个 TUI 删 `credentials.json`，另一个进程下次 EnsureValidToken 就失败 → 进 `LoggedOut`。这是 deliberate（用户表达了"想注销"的意图）。

#### 5.12.7 实现要点

新文件 `internal/agent/tui/auth.go`：
...
type AuthController struct {
    ...
    bus         *Bus
    serverURL   string         // 来自 --server flag 或 saved creds
    skipBrowser bool
    onChange    func(AuthState)  // notify TUI
}

func (a *AuthController) StartLogin(ctx context.Context) tea.Cmd  // 异步启动 device flow
func (a *AuthController) CancelLogin()                              // 取消进行中的 login
func (a *AuthController) Logout() error                             // 清 creds
func (a *AuthController) EnsureValid(ctx context.Context) (string, error)  // refresh + return token
```

`AuthController` 是唯一持有 `*agent.Credentials` 的地方；`Bus` 通过 `auth.EnsureValid()` 拿 token，永远不直接读 creds 文件。这样状态机集中管理，不会出现"两处都在 refresh"的 race。

**复用 `internal/agent/login.go`：** `requestDeviceCode` / `pollForToken` 函数提取为可复用的（已经差不多是这样）。`RunLogin` 函数自身不复用——它是 stdout 输出版本，TUI 走自己的 message-driven 版本。

#### 5.12.8 时序：未登录启动 + /login

```
User      TUI(BubbleTea)        AuthController         agentserver
 │             │                       │                      │
 │  start `tui --server URL`            │                      │
 ├────────────►│                       │                      │
 │             │ load creds → 无       │                      │
 │             │ AuthState = LoggedOut │                      │
 │  render: status bar "auth: logged out"                     │
 │           input disabled, hint "Use /login to authenticate"│
 │◄────────────┤                       │                      │
 │             │                       │                      │
 │ type "/login" + Enter              │                      │
 ├────────────►│                       │                      │
 ...
 │             │ browser.OpenURL       │                      │
 │             │                       │ pollForToken         │
 │             │                       │ POST /api/oauth2/token (every 5s)
 │             │                       ├─────────────────────►│
 ...
 │             │ 触发延迟启动序列     │                      │
 │             │ ├─ ExecutorClient 启动                       │
 │             │ ├─ FetchLatestSession (if --continue)        │
 │             │ └─ SubscribeSSE                              │
 │  render: status bar "auth: <user> · executor: online"     │
 │           input enabled                                    │
 │◄────────────┤                       │                      │
```
...
|---|---|---|---|
| TUI OAuth token 即将过期 | 后台 refresh，AuthState 短暂 Refreshing | 自动 refresh 完成回 LoggedIn | 状态栏短暂 "auth: <user> (refreshing)" |
| TUI OAuth token refresh 失败 | inbound 401 → 清 creds | AuthState 转 LoggedOut，等用户 /login | banner "Token expired. Use /login to re-authenticate." + 状态栏 "auth: logged out" |
| TUI 启动时无 creds | 不阻塞，进 LoggedOut 态 | 等用户 /login | 输入框禁用 + 提示 "Use /login to authenticate" |
...
internal/agent/tui/                                # 新包
  ...
  login_panel.go                                   # OAuth Device Flow 浮层 + QR 渲染（§5.12）
  logout_panel.go                                  # logout 确认面板
  auth.go                                          # AuthController 状态机（§5.12.7）
  cmds.go                                          # slash 命令解析
...
internal/agent/login.go                            # 提取 requestDeviceCode/pollForToken 为可复用
                                                   #（TUI auth.go 调用，不走 stdout）
